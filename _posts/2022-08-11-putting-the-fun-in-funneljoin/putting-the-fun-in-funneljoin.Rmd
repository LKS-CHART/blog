...
description: |
  An introduction to, an example of, and my (lazy) journey to discovering the `funneljoin` package
author:
  - name: Derek Beaton
...
ADT_EVENTS <- read.csv( here::here("_posts","2022-08-11-putting-the-fun-in-funneljoin", "ALL_ADT_EVENTS.csv") )
...
SPECIFIC_PROCEDURES <- read.csv( here::here("_posts","2022-08-11-putting-the-fun-in-funneljoin", "SPECIFIC_PROCEDURES.csv") )
...
The first row is the *procedure* and the second row is *the event that happens after specific procedure*. At the end of what we do, we want all of those *events after procedures* because we needed to understand more about patient movements after procedures for some of our modelling. 
...
```{r empty_join, echo = TRUE}


dim(join_attempt_one)
...
And `funneljoin` is also fun and you can tell because it's literally in the name.
