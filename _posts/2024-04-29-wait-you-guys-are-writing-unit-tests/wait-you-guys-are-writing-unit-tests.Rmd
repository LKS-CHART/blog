...
> [Data Virtuality (DV)](https://datavirtuality.com/) is our data integration platform which consolidates data from various hospital source systems. It offers many advantages:
>
> -   DV has a layer of abstraction between source systems and data consumers. This means that, as a data scientist, I can access real-time data from one view.
>
...
Basically, the above unit test checks that sending a bad SQL query through the DV API should return a 400 status code.
...
We updated the way in which we handle errors. Instead of just checking a response's status code, we also need to check the response body.
...
-   What does a unit test for a *model* looks like?

It's difficult to answer these questions[^2].

[^2]: Perhaps we can try answering these in a future blog post 😉

### Continuous integration?

Currently, I need to run my unit tests **manually**. I regularly do this before pushing changes. It's been working (and hey, in this particular instance, it uncovered something serious!!), but it's not actually *enforced*[^3]. We're using the honor code here.

[^3]: Pre-commit/pre-push hooks could help here, but they also rely on the honor code.
