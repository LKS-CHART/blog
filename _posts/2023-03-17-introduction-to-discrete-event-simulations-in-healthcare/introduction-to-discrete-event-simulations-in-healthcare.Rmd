...
<!-- removed for now because it doesn't quite fit + breaks the flow -->
<!-- ## DES software -->
...
<!-- Our group---Data Science and Advanced Analytics---had a specific project (more details below) where we wanted to host an application for users to enter patient schedules and run the simulation remotely and on-demand. We chose SimPy to accomplish this. -->
...
<!-- Neil fill in here... -->
<!-- We can use DES to test changes without impacting current workflows and operations.
We can simulate in order to uncover improvements. Once we find something that works better, we can then implement it. So DES allows us to explore many possibilities to find a solution.

In our case, we were able to increase the number of appointments while maintaining staffing levels and maintaining social distancing requirements. We did that with DES to help tweak schedules for appointments. 
-->
